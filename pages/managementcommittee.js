import React from "react"
//import Layout from "../components/layout"
import FooterPage from "../components/footer"
import HeaderPage from "../components/header"
import "../styles/app.css"
import "../styles/modal.css"
import "../styles/management.css"
import "../styles/flex.scss"
import "../styles/reboot.scss"
import "../styles/home.css"
import "../styles/index.css"
import "../styles/aos.css"
import "../styles/footer.css"
import "../styles/grid.scss"
import "../styles/display.scss"
import "../styles/spacing.scss"
import "../styles/StrategicPartners.css"
import "../styles/supportingpartner.css"


const ManagementPage = () =>{
	return(
		<div>
		<HeaderPage/>
	<div class="container App mcCont">
	<div></div>
	<div class="row">
	<div class="col-12  d-flex justify-content-center align-items-center">
	<h1 class="mcHeading">Management Committee</h1>
	</div>
	<div class="col-lg-4 col-sm-6 mt-5 d-flex justify-content-center align-items-center">
	<div class="cardBody ">
	<div class="imgHead">
	<div class="greenCir"></div>
	<div class="whiteCir bg1"></div>
	<div class="imgDiv"></div>
	</div>
	<h1>Kazi Rahat Ali</h1><p>General Secretary PIAIC</p>
	<div class="spans d-flex justify-content-center align-items-center">
	<div><ul><li> </li><li> </li><li> </li></ul></div></div><br/></div></div>
	<div class="col-lg-4 col-sm-6 mt-5 d-flex justify-content-center align-items-center">
	<div class="cardBody ">
	<div class="imgHead">
	<div class="greenCir">
	</div>
	<div class="whiteCir zia">
	</div><div class="imgDiv">
	</div></div>
	<h1>Zia Ullah Khan</h1><p>CEO - Panacloud Pvt. Ltd.</p>
	<div class="spans d-flex justify-content-center align-items-center">
	<div><ul><li> <a target="_blank" rel="noopener noreferrer" href="https://www.facebook.com/ziakhan"><img alt="fb-icon" class="span" src="https://www.piaic.org/static/media/fb-logo.6163476d.png"/></a></li><li> <a target="_blank" rel="noopener noreferrer" href="https://twitter.com/ziakhan"><img alt="twitter-icon" class="span" src="data:image/png;base64,"/></a></li><li> <a target="_blank" rel="noopener noreferrer" href="https://www.linkedin.com/in/ziaukhan/"><img alt="linkedin-icon" class="span" src="https://www.piaic.org/static/media/linkedin-icon.6976c96a.png"/></a></li></ul>
	</div>
	</div>
	<br/>
	</div>
	</div>
	<div class="col-lg-4 col-sm-6 mt-5 d-flex justify-content-center align-items-center"><div class="cardBody ">
	<div class="imgHead"><div class="greenCir"></div>
	<div class="whiteCir bg2"></div>
	<div class="imgDiv">
	</div>
	</div><h1>Yousuf Lakhani</h1><p>President - Saylani Welfare Trust<br/><br/></p>
	<div class="spans d-flex justify-content-center align-items-center">
	<div><ul><li> <a target="_blank" rel="noopener noreferrer" href="https://web.facebook.com/saylaniwelfare/"><img alt="fb-icon" class="span" src="https://www.piaic.org/static/media/fb-logo.6163476d.png"/></a></li><li> </li><li> </li></ul></div>
	</div>
	<br/>
	</div>
	</div>
	<div class="col-lg-4 col-sm-6 mt-5 d-flex justify-content-center align-items-center">
	<div class="cardBody ">
	<div class="imgHead">
	<div class="greenCir"></div>
	<div class="whiteCir bg3">
	</div>
	<div class="imgDiv">
	</div>
	</div>
	<h1>Sulaiman Mehdi</h1><p>Chairman of The Board - Pakistan Stock Exchange</p>
	<div class="spans d-flex justify-content-center align-items-center">
	<div><ul><li> </li><li> </li><li> <a target="_blank" rel="noopener noreferrer" href="https://www.linkedin.com/in/sulaiman-s-mehdi-fcis-44275773/"><img alt="linkedin-icon" class="span" src="https://www.piaic.org/static/media/linkedin-icon.6976c96a.png"/></a></li></ul>
	</div>
	</div>
	<br/>
	</div>
	</div>
	</div>
	<br/>
	<br/>
	<br/>
	<br/>
	<hr/>
	<br/>
	<br/>
	<div class="">
	<div class="container App supportingCont"><div class="row">
	<div class="col-12 mt-2 d-flex justify-content-center align-items-center"><p>STRATEGIC PARTNERS</p>
	</div>
	<div class="col-12 mt-4 mb-4 supporterIcons d-flex justify-content-center align-items-center">
	<div class="management">
	<ul><a href="https://www.panacloud.ai/" target="_blank" rel="noopener noreferrer"> <li><img src="https://www.piaic.org/static/media/panacloudLogo.9b5ef574.svg" alt="panacloud-logo"/></li> </a><a href="https://www.psx.com.pk/" target="_blank" rel="noopener noreferrer"> <li><img src="https://www.piaic.org/static/media/psx2.c50b5ae4.png" alt="pak-stock-logo"/></li></a><a href="http://www.saylaniwelfare.com/" target="_blank" rel="noopener noreferrer"> <li><img src="https://www.piaic.org/static/media/saylaniLogo.c2a52d0a.png" alt="saylani-logo"/></li></a></ul>
	</div>
	</div>
	</div>
	</div>
	</div>
	</div>
	<br/>
	<br/>
	<br/>
	<br/>
	<br/>
	<FooterPage/>
	</div>
		)
}
export default ManagementPage